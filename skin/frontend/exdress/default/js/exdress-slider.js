...
;(function($) {
	$.fn.exSlider = function(params) {
		...
		//Vertical Stripes
		function VertStripes(imgslider, areaWidth, areaHeight, stripeSize, bgColor, duration, delay) {
			var $stripes;
			var $arr;
			var total;
			var intervalId = null;
			...
			var init = function() {
				...
				imgslider.addToScreen(divs);
				
				$stripes = imgslider.$el.find("div.vpiece");
				$arr = new Array(total);
				$stripes.each(
					function(n) {						
			...
			var animate = function($img, effect) {
				...
				intervalId = setInterval(
					function() {
						$stripes.eq(start).animate({top:0, opacity:1, width:stripeSize}, duration, imgslider.easing(),
							function() {
								if ($(this).attr("id") == end) {
									imgslider.setComplete($img);
								}
							}
						);
						if (start == end) {
			...
			var animateRandom = function($img) {		
				...
				intervalId = setInterval(
					function() {
						$arr[i++].animate({opacity:1}, duration, imgslider.easing(),
								function() {
									if (++count == total) {
										imgslider.setComplete($img);
									}
		...
		//Horizontal Stripes
		function HorzStripes(imgslider, areaWidth, areaHeight, stripeSize, bgColor, duration, delay) {
			var $stripes;
			var $arr;
			var total;
			var intervalId = null;
			...
			var init = function() {			
				...
				$stripes = imgslider.$el.find("div.hpiece");
				$arr = new Array(total);
				$stripes.each(
					function(n) {
			...
			var animate = function($img, effect) {
				...
				intervalId = setInterval(
					function() {
						$stripes.eq(start).animate({left:0, opacity:1, height:stripeSize}, duration, imgslider.easing(),
							function() {
								if ($(this).attr("id") == end) {
									imgslider.setComplete($img);
								}
		...
		//class Blocks
		function Blocks(imgslider, areaWidth, areaHeight, blockSize, bgColor, duration, delay) {
			var $blockArr;
			var $blocks;
			var $arr;
			var numRows;
			...
			var init = function() {
				...
				imgslider.addToScreen(divs);
				$blocks = imgslider.$el.find("div.block");
			...
			var diagAnimate = function($img, props, rev) {
				...
				intervalId = setInterval(
					function() {
						$array[count++].animate(props, duration, imgslider.easing(),
								function() {
			...
			var vertZigZag = function($img, effect) {
				...
				intervalId = setInterval(
					function() {
						$blockArr[i][j].animate({opacity:1}, duration, imgslider.easing(),
								function() {
									if ($(this).attr("id") == lastId) {
										imgslider.setComplete($img);
									}});
			...
			var dirAnimate = function($img, effect) {
				...
				intervalId = setInterval(
					function() {
						$array[count++].animate({width:blockSize, height:blockSize}, duration, imgslider.easing(),
								function() {
									if ($(this).attr("id") == lastId) {
										imgslider.setComplete($img);
									}
			...
			var randomAnimate = function($img) {
				...
				intervalId = setInterval(
					function() {
						$arr[i].animate({top:$arr[i].data("top"), width:blockSize, height:blockSize, opacity:1}, duration, imgslider.easing(),
								function() {
									if (++count == total) {
										imgslider.setComplete($img);
									}
			...
			var spiral = function($img, spiralOut) {			
				...
				if ($array.length > 0) {
					...
					intervalId = setInterval(
						function() {
							$array[k].animate({opacity:1}, duration, imgslider.easing(),
								function() {
									if ($(this).attr("id") == lastId) {
										imgslider.setComplete($img);
									}
		...
		function Rotator($obj, opts) {
			...
			//init imgslider
			this.init = function() {
				$imgslider = $obj.find(".exdress-slider");
				$screen = $imgslider.find("div.screen");
				$cpanel = $imgslider.find("div.c-panel");
				...
				$imgslider.css({width:screenWidth, height:screenHeight});
				//init components
				initScreen();				
				initButtons();
				initItems();
				...
				if (textMousover) {
					$imgslider.hover(displayText, hideText);
				}
				else {
					$imgslider.bind(UPDATE_TEXT, updateText);
				}
			...
			var initCPanel = function() {	
				if (displayThumbs || displayDBtns || displayPlayBtn) {
					if (cpPos == INSIDE) {
						...
						if (cpMouseover) {
							$imgslider.hover(displayCPanel, hideCPanel);
						}
					}
					else {
						switch (ALIGN[cpAlign]) {
			...
			var setOutsideHP = function(top) {
				...
				$cpWrapper = $imgslider.find(".outer-hp");
				...
				$imgslider.css({height:screenHeight + $cpWrapper.outerHeight()});
			}
			
			var setOutsideVP = function(left) {
				$cpanel.wrap("<div class='outer-vp'></div>");
				$cpWrapper = $imgslider.find(".outer-vp");
				...
				$imgslider.css({width:screenWidth + $cpWrapper.outerWidth()});
			}
			...
			var initButtons = function() {
				...
				if (displaySideBtns) {
					...
					if (cpMouseover) {
						...
						$imgslider.hover(showSideButtons, hideSideButtons);
					}
				}
				...
				if (displayThumbs) {
					...
					if (cpVertical) {
						...
						if ($thumbPanel.height() > maxHeight) {
							...
							$imgslider.bind(UPDATE_LIST, function() {
								if(!$list.is(":animated")) {								
			...
			var initTooltip = function() {
				if (tipType == "text") {
					$("body").append("<div id='imgslider-tooltip'><div class='tt-txt'></div></div>");
					$tooltip = $("body").find("#imgslider-tooltip");
					...
					var content = "<div id='imgslider-tooltip'>";
			...
			var displayText = function() {
				$imgslider.unbind(UPDATE_TEXT).bind(UPDATE_TEXT, updateText).trigger(UPDATE_TEXT);
			}

			//hide text panel on mouseovers
			var hideText = function() {
				$imgslider.unbind(UPDATE_TEXT);
				resetText();
			}
			
			//load current content
			var loadContent = function(i) {
				$imgslider.trigger(UPDATE_LIST);
				if (playOnce) {
					pauseLast(i);
				}
		...
		return this.each(
			function() {
				var imgslider = new Rotator($(this), opts);
				imgslider.init();
			}
		);
	}
