...
class Amasty_Shopby_Model_Mysql4_Filter extends Mage_Core_Model_Mysql4_Abstract
{
    ...
    public function addFilterAttribute($attributeId)
    {
        $db = $this->_getReadAdapter();
        ...
        $sql
            ->joinInner(array('ca' => $this->getTable('catalog/eav_attribute')), 'a.attribute_id = ca.attribute_id', array())
            ->where('ca.is_filterable > 0')
            ->where('a.attribute_id IN (?)', array($attributeId))
        ;
        ...
        $sql = $db->select()->from(array('a'=>$this->getTable('amshopby/filter')), array('a.filter_id'))
        ->where('a.attribute_id IN (?)', array($attributeId));
        $res = $db->fetchAll($sql);
        $filter_id = null;
        foreach($res as $row){
