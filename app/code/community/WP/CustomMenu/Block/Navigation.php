...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    private $_colors = array(
        "#ff6ff2", "#00d5f2", "#008141", "#0715cd", "#b536da", "#000056", "#005682",
        "#77003c", "#004182", "#008282", "#ffb100", "#078446", "#4ac925", "#1f9400",
        "#39e8ec", "#c41a5e", "#5d2892", "#4876b9", "#2b0057",  "#f2a400","#626262"
    );
    ...
    public function drawCustomMenuMobileItem($category, $level = 0, $last = false)
    {
        ...
        // --- Top Menu Item ---drawCustomMenuItem
        $html[] = '<a href="' . $catUrl .'">';
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        if ($drawPopup) {
            $popupEvents = sprintf(
                'onmouseover="wpShowMenuPopup(this, event, \'popup%1$d\');"'
                . ' onmouseout="wpHideMenuPopup(this, event, \'popup%1$d\', \'menu%1$d\')"',
                $id
            );
        }
        $html[] = sprintf('<div id="menu%d" class="menu%s" %s>', $id, $active, $popupEvents);
        // --- Top Menu Item ---
        $color = array_pop($this->_colors);
        $html[] = sprintf('<style type="text/css"> #popup%1$d{background: %2$s;} #menu%1$d a{ color: %2$s;} #menu%1$d.active {background: %2$s;}</style>', $id, $color);
        $html[] = '<div class="parentMenu">';
